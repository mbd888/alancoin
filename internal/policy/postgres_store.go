...
func (p *PostgresStore) Create(ctx context.Context, sp *SpendPolicy) error {
	...
	_, err = p.db.ExecContext(ctx, `
		INSERT INTO spend_policies (id, tenant_id, name, rules, priority, enabled, created_at, updated_at)
...
func (p *PostgresStore) Get(ctx context.Context, id string) (*SpendPolicy, error) {
	row := p.db.QueryRowContext(ctx, `
		SELECT id, tenant_id, name, rules, priority, enabled, created_at, updated_at
...
func (p *PostgresStore) List(ctx context.Context, tenantID string) ([]*SpendPolicy, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&sp.ID, &sp.TenantID, &sp.Name, &rulesJSON,
			&sp.Priority, &sp.Enabled, &sp.CreatedAt, &sp.UpdatedAt); err != nil {
			return nil, err
		}
...
func (p *PostgresStore) Update(ctx context.Context, sp *SpendPolicy) error {
	...
	result, err := p.db.ExecContext(ctx, `
		...
		SET name = $1, rules = $2, priority = $3, enabled = $4, updated_at = $5
		WHERE id = $6 AND tenant_id = $7`,
		sp.Name, rulesJSON, sp.Priority, sp.Enabled, sp.UpdatedAt, sp.ID, sp.TenantID,
	)
	if err != nil {
		if pqErr, ok := err.(*pq.Error); ok && pqErr.Code == "23505" {
			return ErrNameTaken
...
func scanPolicy(row *sql.Row) (*SpendPolicy, error) {
	...
	err := row.Scan(&sp.ID, &sp.TenantID, &sp.Name, &rulesJSON,
		&sp.Priority, &sp.Enabled, &sp.CreatedAt, &sp.UpdatedAt)
